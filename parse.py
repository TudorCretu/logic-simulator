...
class Parser:
    ...
    def print_msg(self, success):
        ...
        if success is True:
            ...
            print("File '%s' contains %d error(s): %d syntax error(s) and %d semantic error(s)\n"%(self.scanner.file.name.split('/')[-1], self.error_count, self.error_count-self.semerr_count,self.semerr_count))
            self.error_to_gui.append("File '%s' contains %d error(s): %d syntax error(s) and %d semantic error(s)\n"%(self.scanner.file.name.split('/')[-1], self.error_count, self.error_count-self.semerr_count,self.semerr_count))
